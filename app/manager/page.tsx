...
import { formatDuration, isTicketOverdue, minutesBetween } from "@/lib/time";
...
export default function ManagerPage() {
  ...
  const processing = tickets.filter((ticket) => ticket.status === "accepted" || ticket.status === "processing").length;
  const timeout = tickets.filter((ticket) => ticket.status === "timeout" || isTicketOverdue(ticket.due_at, ticket.status)).length;
  const avgAccept = average(tickets.map((ticket) => minutesBetween(ticket.created_at, ticket.accepted_at)).filter(isNumber));
  const avgComplete = average(
    completed.map((ticket) => minutesBetween(ticket.accepted_at ?? ticket.created_at, ticket.completed_at)).filter(isNumber)
  );
