...
export function TicketTable({ tickets }: { tickets: Ticket[] }) {
  return (
    <div className="overflow-x-auto rounded-3xl bg-white/85 shadow-panel">
      <Table className="min-w-[920px]">
        <thead className="bg-slate-950/5">
          <tr>
            <Th className="w-24">房号</Th>
            <Th className="min-w-[220px]">需求</Th>
            <Th className="w-28">部门</Th>
            <Th className="w-28">状态</Th>
            <Th className="w-32">负责人</Th>
        ...
        <tbody>
          {tickets.map((ticket) => (
            <tr key={ticket.id}>
              ...
              <Td className="min-w-[220px] font-medium">{ticket.request}</Td>
